...
####### unlike the script provided in the appendix, this script########
####### can allow for users to look at any scenarios and number########
####### of scenarios with the following data files             ########
####### using the following data files available with this     ########
####### script: randomData.csv, scenarioData.csv,              ########
####### groupComparisionData.csv                               ########
########################################################################
...
wd.out <- "c:\\Users\\hkropp\\Google Drive\\mult_comp\\sub_out2"
#path for model code
...
#################################
#read in all data
datMT <- read.csv(paste0(wd.data,"\\randomData.csv"))
scnDataA <- read.csv(paste0(wd.data,"\\scenarioData.csv"))
groupCompD <-  read.csv(paste0(wd.data,"\\groupComparisionData.csv"))
...
for(i in 1:numsims){
	...
	scnD[[i]] <- scnDataA[scnDataA$scenarioid==simN[i],]
}
